function analyzeResults2(sp,uselog)

global  t position dz_dt d2z_dt2 d3z_dt3 Af As dA_dt filePath Su_ Su alfa Karlovitz tWalls sheet editing simulating numPointsAnalysis
figure(1);
...
if(strcmp('Y',reply) || strcmp('y',reply))
    ...
    t_max_dz_dt=fsolve(@(x)feval(d2k_dt2,x),tx(pos_max_dz_dt));
    
    [miniii iii]=min(abs(tx-t_max_dz_dt));
    t_max_dz_dt=tx(iii);
